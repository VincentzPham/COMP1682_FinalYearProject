...
#     ### Begin Manual LSTM Model Training ###
#     st.write("**Manual LSTM Model Training**")
   
#     # Define the Manual LSTM model
#     manual_lstm_model = Sequential()
#     manual_lstm_model.add(LSTM(
#         units=50,  # Predefined number of units
#         activation='relu',
#         return_sequences=False,
#         input_shape=(X_train.shape[1], X_train.shape[2])
#     ))
#     manual_lstm_model.add(Dense(1))
   
#     # Compile the LSTM model
#     manual_lstm_model.compile(
#         optimizer=Adam(learning_rate=0.001),
#         loss='mean_squared_error'
#     )
   
#     st.write("**Training Manual LSTM Model...**")
   
#     # Train the LSTM model
#     with st.spinner('Training Manual LSTM model...'):
#         manual_lstm_history = manual_lstm_model.fit(
#             X_train, y_train,
#             epochs=100,
#             batch_size=32,
#             validation_split=0.1,
...
#             verbose=0
#         )
#     st.success('Manual LSTM model training completed!')
   
#     # Plot Manual LSTM training and validation loss
#     st.subheader("Manual LSTM Training and Validation Loss")
#     plt.figure(figsize=(10,6))
#     plt.plot(manual_lstm_history.history['loss'], label='Training Loss')
#     plt.plot(manual_lstm_history.history['val_loss'], label='Validation Loss')
#     plt.title('Manual LSTM Loss Over Epochs')
#     plt.xlabel('Epoch')
#     plt.ylabel('Mean Squared Error')
#     plt.legend()
#     plt.grid(True)
...
#     # Save the trained Manual LSTM model
#     manual_lstm_model_path = 'manual_lstm_trained.keras'
#     manual_lstm_model.save(os.path.join(output_dir_model, 'manual_lstm_trained.keras'), save_format='keras')
#     st.write(f"**Trained Manual LSTM model saved as `{manual_lstm_model_path}`**")
   
#     # Predicting and inverse transforming the Manual LSTM predictions
#     manual_lstm_predicted = manual_lstm_model.predict(X_test)
#     manual_lstm_predicted = scaler.inverse_transform(manual_lstm_predicted)
   
#     # Inverse transform the actual values for comparison
#     actual = scaler.inverse_transform(y_test.reshape(-1, 1))
   
#     # Visualization of Manual LSTM predictions vs actual
#     st.subheader("Manual LSTM Prediction vs Actual")
#     plt.figure(figsize=(10,6))
#     plt.plot(actual, color='blue', label='Actual')
#     plt.plot(manual_lstm_predicted, color='green', linestyle='--', label='Manual LSTM Predicted')
#     plt.title(f'Manual LSTM Prediction for {selected_country} - {selected_column}')
#     plt.xlabel('Time Steps')
#     plt.ylabel(f'{selected_column} (in thousands of metric tons)')
#     plt.legend()
#     plt.grid(True)
...
#     # Calculate Manual LSTM metrics
#     manual_lstm_mape = mean_absolute_percentage_error_custom(actual, manual_lstm_predicted)
#     manual_lstm_mae = mean_absolute_error_metric(actual, manual_lstm_predicted)
#     manual_lstm_rmse = root_mean_squared_error(actual, manual_lstm_predicted)
#     manual_lstm_mse = mean_squared_error_metric(actual, manual_lstm_predicted)

#     # Display Manual LSTM metrics
#     st.subheader("Manual LSTM Evaluation Metrics")
#     manual_lstm_col1, manual_lstm_col2 = st.columns(2)
#     with manual_lstm_col1:
#         if np.isnan(manual_lstm_mape):
#             st.metric("MAPE", "Undefined")
#         else:
#             st.metric("MAPE", f"{manual_lstm_mape:.2f}%")
#         st.metric("MAE", f"{manual_lstm_mae:.2f}")
#     with manual_lstm_col2:
#         st.metric("RMSE", f"{manual_lstm_rmse:.2f}")
#         st.metric("MSE", f"{manual_lstm_mse:.2f}")

#     # Save Manual LSTM evaluation metrics and predictions
#     with st.spinner('Saving Manual LSTM evaluation metrics and predictions...'):
#         # Round metrics to 2 decimal places before saving
#         manual_lstm_evaluation_metrics = pd.DataFrame({
#             'RMSE': [round(manual_lstm_rmse, 2)],
#             'MAE': [round(manual_lstm_mae, 2)],
#             'MSE': [round(manual_lstm_mse, 2)],
#             'MAPE': [round(manual_lstm_mape, 2) if not np.isnan(manual_lstm_mape) else 'Undefined']
#         })
#         manual_lstm_evaluation_metrics.to_csv(os.path.join(output_dir_result, 'manual_lstm_evaluation_metrics.csv'), index=False)
#         st.write("**Manual LSTM Evaluation metrics saved as `manual_lstm_evaluation_metrics.csv`**")
    
#         # Save Manual LSTM prediction results
#         manual_lstm_results_df = pd.DataFrame({
#             'True Values': actual.flatten(),
#             'Manual LSTM Predictions': manual_lstm_predicted.flatten()
#         })
#         manual_lstm_results_df.to_csv(os.path.join(output_dir_result, 'manual_lstm_predictions_vs_real.csv'), index=False)
#         st.write("**Manual LSTM Prediction results saved as `manual_lstm_predictions_vs_real.csv`**")
   
#     ### End of Manual LSTM Model Training ###
   
#     ### Begin LSTM Model Training with Hyperparameter Tuning ###
...
# Set page configuration
st.set_page_config(
    page_title="CO2 Emissions Deep Learning",
    layout="wide",
    initial_sidebar_state="expanded",
)
...
# Sidebar for country and column selection is removed as selections are defaulted
...
# Set default country and column
...
if country_data.empty:
    st.warning(f"No data available for {selected_country}. Please check the dataset.")
    st.stop()
...
# Check if 'Total' is in numerical columns; if not, handle accordingly
if selected_column not in numerical_columns:
    st.warning(f"'{selected_column}' column not found in the dataset for {selected_country}. Please check the dataset.")
    st.stop()
...
scaler_path = os.path.join(output_dir_model, 'scaler_params.pkl')
if not os.path.exists(scaler_path):
    st.error(f"Scaler parameters not found at `{scaler_path}`. Please ensure the scaler has been saved correctly.")
    st.stop()
...
# Normalize the data using loaded scaler
...
### Begin Simple RNN Model Evaluation ###
st.write("**Simple RNN Model Evaluation**")

# Load the Simple RNN model
rnn_model_path = os.path.join(output_dir_model, 'simple_rnn_trained.keras')
if not os.path.exists(rnn_model_path):
    st.error(f"Simple RNN model not found at `{rnn_model_path}`. Please ensure the model has been saved correctly.")
    st.stop()
...
# ax.plot(rnn_predicted, color='red', linestyle='--', label='RNN Predicted')
# ax.set_title(f'Simple RNN Prediction for {selected_country} - {selected_column}')
# ax.set_xlabel('Time Steps')
# ax.set_ylabel(f'{selected_column} (in thousands of metric tons)')
# ax.legend()
# ax.grid(True)
# plt.tight_layout()

# # Save the figure
# plot_path = os.path.join(output_dir_result, 'Simple_RNN_Prediction_vs_Actual.png')
# fig.savefig(plot_path)
# st.write(f"Chart saved at `{plot_path}`")
...
### End of Simple RNN Model Evaluation ###

### Begin Manual LSTM Model Evaluation ###
st.write("**Manual LSTM Model Evaluation**")

# Load the Manual LSTM model
manual_lstm_model_path = os.path.join(output_dir_model, 'manual_lstm_trained.keras')
if not os.path.exists(manual_lstm_model_path):
    st.error(f"Manual LSTM model not found at `{manual_lstm_model_path}`. Please ensure the model has been saved correctly.")
    st.stop()

manual_lstm_model = load_model(manual_lstm_model_path)
...
### End of Manual LSTM Model Evaluation ###

### Begin LSTM Model Evaluation with Hyperparameter Tuning ###
st.write("**LSTM Model with Hyperparameter Tuning Evaluation**")

# Load the best LSTM model
best_lstm_model_path = os.path.join(output_dir_model, 'best_model_trained.keras')
if not os.path.exists(best_lstm_model_path):
    st.error(f"Best LSTM model not found at `{best_lstm_model_path}`. Please ensure the model has been saved correctly.")
    st.stop()

best_lstm_model = load_model(best_lstm_model_path)

# Predicting and inverse transforming the LSTM predictions
predicted = best_lstm_model.predict(X_test)
predicted = scaler.inverse_transform(predicted)
...
# Save LSTM evaluation metrics and predictions
with st.spinner('Saving LSTM evaluation metrics and predictions...'):
    # Round metrics to 2 decimal places before saving
    evaluation_metrics = pd.DataFrame({
        'RMSE': [round(rmse, 2)],
        'MAE': [round(mae, 2)],
