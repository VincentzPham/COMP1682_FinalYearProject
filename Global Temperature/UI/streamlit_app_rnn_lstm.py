...
st.set_page_config(page_title="Global Temperature Deep Learning", layout="wide")
...
# Streamlit App
st.title("📈 LSTM and SimpleRNN Temperature Forecasting for Germany")
st.write("""
This application uses pre-trained LSTM and SimpleRNN models to forecast temperature based on historical data for Germany. 
It includes data preprocessing, model loading, evaluation, and visualization.
""")

@st.cache_data
def load_data(file_path, country='Germany'):
    """Load and filter data for the selected country."""
    df = pd.read_csv(file_path, encoding='latin-1')
    # Check if 'Country' column exists
    if 'Country' not in df.columns:
...
@st.cache_data
def preprocess_data(df, scaler_path='scaler_params.pkl'):
    """Load scaler parameters and scale the temperature data."""
    data = df['TempC'].values
    scaler_path_full = os.path.join(output_dir_models, scaler_path)
    if not os.path.exists(scaler_path_full):
        st.error(f"Scaler parameters file not found: {scaler_path_full}")
        return None, None
    scaler_params = joblib.load(scaler_path_full)
    scaler = MinMaxScaler(feature_range=(0, 1))
...
def main():
    ...
    with st.spinner('📈 Scaling data...'):
        ...
        if scaled_data is None:
            return
        st.success("✅ Data scaled successfully!")
        st.write("**Scaler parameters loaded:**")
        st.json(scaler_params)
    ...
    # =================== Load and Evaluate SimpleRNN Model ===================
    st.header("🤖 SimpleRNN Model Evaluation")
    with st.spinner('🔧 Loading SimpleRNN model...'):
        simple_rnn_model_path = os.path.join(output_dir_models, 'simple_rnn_model_trained.keras')
        if not os.path.exists(simple_rnn_model_path):
            st.error(f"SimpleRNN model file not found: {simple_rnn_model_path}")
            return
        rnn_model = load_model(simple_rnn_model_path)
    ...
    ax_rnn.plot(y_true, label='True Values', color='blue')
    ax_rnn.plot(y_pred_rnn, label='RNN Predictions', color='red')
    ax_rnn.set_title('SimpleRNN: True Values vs Predictions')
    ax_rnn.set_xlabel('Time')
    ax_rnn.set_ylabel('Temperature (°C)')
    ax_rnn.legend()
    ax_rnn.grid(True)
    st.pyplot(fig_rnn)
    plt.close(fig_rnn)
    ...
    st.header("🔮 LSTM Model Evaluation")
    with st.spinner('🔧 Loading LSTM model...'):
        lstm_model_path = os.path.join(output_dir_models, 'best_model_trained.keras')
        if not os.path.exists(lstm_model_path):
            st.error(f"LSTM model file not found: {lstm_model_path}")
            return
        lstm_model = load_model(lstm_model_path)
        st.success("✅ LSTM model loaded successfully!")
    
    with st.spinner('📊 Evaluating LSTM model...'):
        test_loss_lstm = lstm_model.evaluate(X_test_reshaped, y_test, verbose=0)
        st.write(f"**LSTM Test Loss (MSE):** {test_loss_lstm:.4f}")
    ...
    ax_lstm.plot(y_true, label='True Values', color='blue')
    ax_lstm.plot(y_pred_lstm, label='LSTM Predictions', color='green')
    ax_lstm.set_title('LSTM: True Values vs Predictions')
    ax_lstm.set_xlabel('Time')
    ax_lstm.set_ylabel('Temperature (°C)')
    ax_lstm.legend()
    ax_lstm.grid(True)
    st.pyplot(fig_lstm)
    plt.close(fig_lstm)
    ...
    with st.spinner('💾 Saving LSTM results...'):
        ...
        results_lstm_df.to_csv(os.path.join(output_dir_result, 'lstm_temperature_predictions_vs_real.csv'), index=False)
        
        evaluation_metrics_lstm = pd.DataFrame({
            'RMSE': [rmse_lstm], 
            'MAE': [mae_lstm], 
            'MSE': [mse_lstm], 
            'MAPE': [mape_lstm]
        })
        evaluation_metrics_lstm.to_csv(os.path.join(output_dir_result, 'lstm_evaluation_metrics.csv'), index=False)
        st.success("✅ LSTM results saved as 'lstm_temperature_predictions_vs_real.csv' and 'lstm_evaluation_metrics.csv'.")
    ...
    # # Provide download links for RNN
    # st.subheader("🤖 SimpleRNN Results")
    # rnn_predictions_path = os.path.join(output_dir_result, 'rnn_temperature_predictions_vs_real.csv')
    # rnn_metrics_path = os.path.join(output_dir_result, 'rnn_evaluation_metrics.csv')
    # if os.path.exists(rnn_predictions_path):
    #     with open(rnn_predictions_path, 'rb') as f:
    #         st.download_button(
    #             '📂 Download RNN Predictions vs True Values',
    #             f,
    #             'rnn_temperature_predictions_vs_real.csv',
    ...
    #         )
    # else:
    #     st.warning(f"File not found: {rnn_predictions_path}")
    
    # if os.path.exists(rnn_metrics_path):
    #     with open(rnn_metrics_path, 'rb') as f:
    #         st.download_button(
    #             '📊 Download RNN Evaluation Metrics',
    #             f,
    #             'rnn_evaluation_metrics.csv',
    ...
    #         )
    # else:
    #     st.warning(f"File not found: {rnn_metrics_path}")
    
    # # Provide download links for LSTM
    # st.subheader("🔮 LSTM Results")
    # lstm_predictions_path = os.path.join(output_dir_result, 'lstm_temperature_predictions_vs_real.csv')
    # lstm_metrics_path = os.path.join(output_dir_result, 'lstm_evaluation_metrics.csv')
    # if os.path.exists(lstm_predictions_path):
    #     with open(lstm_predictions_path, 'rb') as f:
    #         st.download_button(
    #             '📂 Download LSTM Predictions vs True Values',
    #             f,
    #             'lstm_temperature_predictions_vs_real.csv',
    #             'text/csv'
    #         )
    # else:
    #     st.warning(f"File not found: {lstm_predictions_path}")
    
    # if os.path.exists(lstm_metrics_path):
    #     with open(lstm_metrics_path, 'rb') as f:
    #         st.download_button(
    #             '📊 Download LSTM Evaluation Metrics',
    #             f,
    ...
    #         )
    # else:
    #     st.warning(f"File not found: {lstm_metrics_path}")
